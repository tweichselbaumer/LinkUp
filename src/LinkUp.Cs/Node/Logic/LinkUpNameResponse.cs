...
namespace LinkUp.Cs.Node.Logic
{
   internal class LinkUpNameResponse : LinkUpLogic
   {
      ...
      protected override void ParseFromRaw(byte[] data)
      {
         ...
         ushort stringLength = BitConverter.ToUInt16(data, 4);
         Name = Encoding.UTF8.GetString(data, 6, stringLength);
      }

      protected override byte[] ToRaw()
      {
         return new byte[] { (byte)LinkUpLogicType.NameResponse, (byte)LabelType }.Concat(BitConverter.GetBytes(Identifier)).Concat(BitConverter.GetBytes((ushort)Name.Length)).Concat(Encoding.UTF8.GetBytes(Name)).ToArray();
      }
   }
}