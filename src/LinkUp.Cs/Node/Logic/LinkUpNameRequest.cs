...
namespace LinkUp.Cs.Node.Logic
{
   internal class LinkUpNameRequest : LinkUpLogic
   {
      ...
      protected override void ParseFromRaw(byte[] data)
      {
         ...
         ushort stringLength = BitConverter.ToUInt16(data, 2);
      ...
      {
         return new byte[] { (byte)LinkUpLogicType.NameRequest, (byte)LabelType }.Concat(BitConverter.GetBytes((ushort)Name.Length)).Concat(Encoding.UTF8.GetBytes(Name)).Concat(Options).ToArray();
      }
   }
}